...
public void parseData(){//parse data from downloaded file and upload them to arrayList guestList
//String[] myArray;
...
public boolean getCompany(int j){
	if(company[j]+1 <= 10)
	{
		company[j]++;
		return true;
...
public int setCompanyMem(int company, int index){//initialize companyMulti with the member's id
...
public ArrayList<Attendee> removePeople(ArrayList<Company> company, ArrayList<ArrayList<Integer>> companyMulti){//remove extra people starting with the smallest id number from that company
 boolean filler = true;
   ...
   if(company.get(k).getCompanyNum() > 10){
     ...
     for(int i = 0; i < extraPeople; i++){//remove extra people from guestList starting with the smallest id
       for(int h = 0; h < guestList.size(); h++){
         if(guestList.get(h).getCompany() == k){
...
/*
public void placePeople(ArrayList<ArrayList<Integer>> companyMulti, int company, ArrayList<Attendee> guestList){//place people into tables and seats
  boolean filler = false;
 int tableNum = 0;
 int seatNum = 0;
 for(int i = 0; i < companyMulti.get(company).size(); i++){
   ...
   for(int y = 0; y < guestList.size(); y++){
     if(guestList.get(y).getID() == companyMulti.get(company).get(i)){
       index = y;
       break;
     }
     else{
...
public Attendee getPerson(int id){//get member's info with inputted id 
Attendee a1 = new Attendee(0, "0", "0", 0, 0, 0);
 ...
 return(a1);
	...
	public void setTable(int[][] setTable)
	{
		table = setTable;
	}
